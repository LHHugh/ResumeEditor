...
[![Version](https://img.shields.io/badge/version-v1.4.3-blue)](CHANGELOG)
...
| **模块管理** | 大模块自由增减（教育 / 工作 / 项目 / 技能 / 更多信息 / 作品 / 评价）；**长按拖动调整主区域 / 侧边栏模块顺序**；自定义模块支持五种内部结构 |
...
- **自定义模块**：输入名称即可新增。新增时可选择内部结构：
  ...
  - **教育背景格式**：每条含「时间起止 + 学校 + 专业 + 学历」（适用于培训经历、交流学习等）；
  - **项目经验格式**：每条含「项目名称 + 角色 + 时间 + 简介 + 内容」（适用于开源项目、课外项目等）；
  - **个人评价格式**：一段多行评价文字（适用于自我描述、座右铭等）；
  - **个人技能格式**：分组式技能标签（适用于技能墙、工具清单等）。
- **侧边栏模块**：在侧边栏模板下，可在「模块管理 → 侧边栏模块」单独增删，同样支持上述五种格式。
...
| 版本 | 日期 | 主要变更 |
| --- | --- | --- |
| **v1.4.3** | 2026-09 | 自定义模块新增「**个人评价格式**」（一段多行评价文字，主区域与侧栏通用，与内置个人评价同版式） |
| **v1.4.2** | 2026-09 | 自定义模块新增「**教育背景格式**」「**项目经验格式**」（主区域与侧栏通用），格式随数据持久化、条目删空不丢失 |
